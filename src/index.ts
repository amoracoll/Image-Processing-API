...
app.get("/resize", async (req: Request, res: Response): Promise<any> => {
  ...
  if (
    ...
    !height ||
    !fileName ||
    !validateParams(width as string, height as string, fileName as string)
  ) {
    return res
      .status(400)
      .send(
        "Faltan parámetros o son inválidos. Debe proporcionar imageName, width y height.",
      );
  }
  ...
  try {
    ...
    if (isNaN(widthNumber) || isNaN(heightNumber)) {
      return res
        .status(400)
        .send("Los parametros width y height deben ser numeros validos");
    }

    // Redimensionar la imagen
    const resizedImage = await sharp(imagePath)
      .resize(Number(widthNumber), Number(heightNumber))
      .toBuffer();
...
app.get(
  "/placeholder/:width(\\d+)x:height(\\d+)",
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      res
        .status(500)
        .send("Error al generar la imagen de marcador de posición.");
    }
  },
);
